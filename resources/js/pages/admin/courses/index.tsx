...
export default function CoursesIndex({ courses, programs, filters = {}, perPageOptions = [] }: CoursesIndexProps) {
    ...
    const paginationMeta: PaginationMeta = {
        current_page: courses?.meta?.current_page ?? courses?.current_page ?? DEFAULT_PAGINATION_META.current_page,
        last_page: courses?.meta?.last_page ?? courses?.last_page ?? DEFAULT_PAGINATION_META.last_page,
        per_page: courses?.meta?.per_page ?? courses?.per_page ?? DEFAULT_PAGINATION_META.per_page,
        total: courses?.meta?.total ?? courses?.total ?? DEFAULT_PAGINATION_META.total,
    };
    ...
    }, [filters.search, filters.program, filters.level, filters.visible, filters.per_page, paginationMeta.per_page]);
    ...
    const applyFilters = (event?: FormEvent) => {
        ...
        router.get(CourseController.index().url, buildFilterQuery(), {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
    ...
    const resetFilters = () => {
        ...
        router.get(CourseController.index().url, { per_page: resetState.per_page }, {
            preserveScroll: true,
            replace: true,
        });
    };
    ...
    const goToPage = (page: number) => {
        ...
        router.get(CourseController.index().url, buildFilterQuery({ page }), {
            preserveScroll: true,
            preserveState: true,
            replace: true,
        });
    ...
    return (
        <AppLayout>
            <Head title={isZh ? '課程管理' : 'Courses Management'} />
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <h1 className="text-3xl font-bold tracking-tight text-gray-900">
                                {isZh ? '課程管理' : 'Courses Management'}
                            </h1>
                            <p className="mt-1 text-gray-600">
                                {isZh
                                    ? '檢視與管理系上課程資訊、課綱與顯示狀態'
                                    : 'Review and manage course details, syllabus links, and visibility'}
                            </p>
                        </div>
                                ...
                                <div>
                                    ...
                                    <Select
                                        ...
                                        onChange={(event) => {
                                            ...
                                            router.get(CourseController.index().url, buildFilterQuery({ per_page: value }), {
                                                preserveScroll: true,
                                                preserveState: true,
                                                replace: true,
                                            });
                                            ...
                                            <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                                                <div className="min-w-0 flex-1 space-y-3">
                                                    <div className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                                                        ...
                                                        >
                                                            {course.visible ? (isZh ? '顯示中' : 'Visible') : isZh ? '隱藏' : 'Hidden'}
                                                        </Badge>
                                                    </div>
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-600">
                                                        ...
                                                        {course.semester && (
                                                            <span className="inline-flex items-center gap-2">
                                                                ...
                                                                {isZh ? `開課學期：${course.semester}` : `Semester: ${course.semester}`}
                                                            </span>
                                                        )}
                                                        {course.syllabus_url && (
                                                            <a
