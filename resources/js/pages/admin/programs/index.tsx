...
const LEVEL_LABELS: Record<Program['level'], { zh: string; en: string }> = {
...
export default function ProgramsIndex({ programs, filters = {}, perPageOptions = [] }: ProgramsIndexProps) {
    ...
    const paginationMeta: PaginationMeta = {
        current_page: programs?.meta?.current_page ?? programs?.current_page ?? DEFAULT_PAGINATION_META.current_page,
        last_page: programs?.meta?.last_page ?? programs?.last_page ?? DEFAULT_PAGINATION_META.last_page,
        per_page: programs?.meta?.per_page ?? programs?.per_page ?? DEFAULT_PAGINATION_META.per_page,
        total: programs?.meta?.total ?? programs?.total ?? DEFAULT_PAGINATION_META.total,
    };
    ...
    const applyFilters = (event?: FormEvent) => {
        ...
        router.get(ProgramController.index().url, buildFilterQuery(), {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
    ...
    const resetFilters = () => {
        ...
        router.get(ProgramController.index().url, { per_page: resetState.per_page }, {
            preserveScroll: true,
            replace: true,
        });
    };
    ...
    const goToPage = (page: number) => {
        ...
        router.get(ProgramController.index().url, buildFilterQuery({ page }), {
            preserveScroll: true,
            preserveState: true,
            replace: true,
        });
    ...
    const formatLevelLabel = (level: Program['level']) => {
    ...
    const formatDescription = (program: Program) => {
    ...
    return (
        <AppLayout>
            <Head title={isZh ? '學程管理' : 'Programs Management'} />
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <h1 className="text-3xl font-bold tracking-tight text-gray-900">
                                {isZh ? '學程管理' : 'Programs Management'}
                            </h1>
                            <p className="mt-1 text-gray-600">
                                {isZh
                                    ? '統整系上學程資訊、招生管道與對外網站'
                                    : 'Organize academic programs, related descriptions, and public links'}
                            </p>
                        </div>
                            ...
                            <form onSubmit={applyFilters} className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        {(Object.keys(LEVEL_LABELS) as Array<Program['level']>).map((key) => (
                                ...
                                <div>
                                    ...
                                    <Select
                                        ...
                                        onChange={(event) => {
                                            ...
                                            router.get(ProgramController.index().url, buildFilterQuery({ per_page: value }), {
                                                preserveScroll: true,
                                                preserveState: true,
                                                replace: true,
                                            });
                                                ...
                                                <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                                                    <div className="min-w-0 flex-1 space-y-3">
                                                        <div className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                                                            ...
                                                            >
                                                                {program.visible ? (isZh ? '顯示中' : 'Visible') : isZh ? '隱藏' : 'Hidden'}
                                                            </Badge>
                                                            ...
                                                            <Badge variant="outline" className="flex items-center gap-1">
                                                                ...
                                                                {isZh ? `排序 ${program.sort_order ?? '-'}` : `Order ${program.sort_order ?? '-'}`}
                                                            </Badge>
                                                        </div>
